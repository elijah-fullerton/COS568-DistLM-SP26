...
def sync_gradients_allreduce(args, model):
    """
    Average gradients across ranks using torch.distributed.all_reduce.

    After backward(), each rank has its local gradients. We perform:
      grad_sum = sum over ranks
      grad_mean = grad_sum / world_size
    and write grad_mean back into p.grad on every rank.
    """
...
def train(args, train_dataset, model, tokenizer):
    ...
    if profiling_enabled:
        ...
        trace_path = os.path.join(args.output_dir, f"trace_task4_task2b_all_reduce_{rank_tag}.json")
    ...
    for epoch_idx in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.fp16:
                ...
                if is_distributed:
                    sync_gradients_allreduce(args, model)
    ...
    if is_distributed and args.local_rank == 0 and len(opt_step_times) > 1:
        ...
        print(f"Task2(b) avg opt-step time excl first: {avg_time:.6f}s")
