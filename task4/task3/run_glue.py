...
def train(args, train_dataset, model, tokenizer):
    ...
    if profiling_enabled:
        ...
        trace_path = os.path.join(args.output_dir, f"trace_task4_task3_ddp_{rank_tag}.json")
    ...
    if is_distributed and args.local_rank == 0 and len(opt_step_times) > 1:
        ...
        print(f"Task3 avg opt-step time excl first: {avg_time:.6f}s")
...
def main():
    ...
    # Register model with DistributedDataParallel (Task 3).
    # DDP will synchronize gradients during backward() automatically.
    if args.local_rank != -1:
        if args.device.type == "cuda":
